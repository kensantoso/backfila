...
class SqlDelightDatasourceBackend @Inject constructor(
  ...
  private fun <R : Any, Param : Any> createSqlDelightDatasourceOperator(
    backfill: SqlDelightDatasourceBackfill<R, Param>,
  ...
  override fun create(backfillName: String): BackfillOperator? {
    return getBackfill(backfillName)?.let { backfill ->
      ...
      return createSqlDelightDatasourceOperator(backfill as SqlDelightDatasourceBackfill<Any, Any>)
    }
  }
  ...
  override fun backfills(): Set<BackfillRegistration> {
    return backfills.map {
      BackfillRegistration(
        ...
        parametersClass = parametersClass(it.value as KClass<SqlDelightDatasourceBackfill<Any, Any>>),
        deleteBy = it.value.findAnnotation<DeleteBy>()?.parseDeleteByDate(),
      )
    }.toSet()
  }

  private fun <R : Any, P : Any> parametersClass(backfillClass: KClass<out SqlDelightDatasourceBackfill<R, P>>): KClass<P> {
    // Like MyBackfill.
